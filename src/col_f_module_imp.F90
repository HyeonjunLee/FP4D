...
module col_f_module
use FP4D_globals, only : col_f_nthreads, opt_Col
...
type col_f_sp_type
  ...
  real (kind=8), allocatable, dimension(:,:) :: pdf_np1,pdf_np2 !![get_local_f<setup_col_sp] To be used as PDF at next Picard step. BUT has a role of "df" at end
!  real (kind=8), allocatable, dimension(:,:,:) :: ED   ! 5 x (col_f_nvr-1) x (col_f_nvz-1)
  ...
  !! <SOLVER-related>
  integer, parameter :: vpic_inner_iter_max = 9999

  integer, private, parameter :: MAX_THREADS=64
...
#include "bsolver.F90"
    ...
    !!
    subroutine col_f_setup(isp,nsp,npsi,nth,nmu,nvp,dt,vp_max,smu_max,dvp,dth,dsmu,mype)
      implicit none
      integer, intent(in) :: isp, nsp, nmu, nvp, mype, nth,npsi
      real (kind=8), intent(in) :: dt, vp_max, dvp, smu_max, dsmu, dth
      !
      integer :: i, j
      ...
      do ithread=1,isize
         do isp=col_f_sp_s,col_f_sp_e
             allocate(col_sp_cell_all(isp,ithread)%mesh_r(col_f_nvr), &
                      ...
                      col_sp_cell_all(isp,ithread)%pdf_np2(col_f_nvr, col_f_nvz), &
      ...
      do ithread=1,isize
         do i=col_f_sp_s,col_f_sp_e
            deallocate(col_sp_cell_all(i,ithread)%mesh_r,      col_sp_cell_all(i,ithread)%mesh_z,      &
                       ...
                       col_sp_cell_all(i,ithread)%pdf_n,   col_sp_cell_all(i,ithread)%pdf_np2, &
...
!$acc exit data delete(M_ab)
    ...
    !!
    subroutine setup_col_sp(f_in,inode, mass_in, t_ev_in,  &
      ...
      do isp=col_f_sp_s,col_f_sp_e
        ...
        do iz=1, col_f_nvz
          do ir=1, col_f_nvr
            ...
            col_spall(isp)%pdf_np2(ir,iz) = col_spall(isp)%pdf_n(ir,iz)
          enddo
        enddo
...
!        col_spall(isp)%mesh_z(col_f_nvz)   = col_spall(isp)%vpar_beg + mesh_dz*(col_f_nvz-1)
        col_spall(isp)%mesh_z_half(0)  = col_spall(isp)%vpar_beg + mesh_dz*(-0.5D0)
    ...
    subroutine col_f_lambda_gamma_pair(sp_a, sp_b, gammac_loc)  !! C(a->b)
      use FP4D_globals, only : sml_e_charge, sml_mype, sml_istep, sml_pi
      implicit none
    ...
    !!
    subroutine fm_collision(f_in,df_out,nthreads,den_in,t_ev_in,mass_in,charge_in,&
                 J,invJ,int_J,B,dBdth,dphidth,sml_istep,int_n,int_T,int_p,FSAn,FSAT,FSAp)
      use FP4D_globals, only: sml_mype
#ifdef _OPENMP
      use omp_lib
#endif
      ...
      real(kind=8), intent(in) :: f_in(-col_f_nvzm1/2:col_f_nvzm1/2,0:col_f_nthm1,  &
                                       0:col_f_nvrm1,1:col_f_npsi,col_f_sp_s:col_f_sp_e)
      real(kind=8), intent(out) :: df_out(-col_f_nvzm1/2:col_f_nvzm1/2,0:col_f_nthm1,  &
                                       0:col_f_nvrm1,1:col_f_npsi,col_f_sp_s:col_f_sp_e)
      real(kind=8), dimension(-col_f_nvzm1/2:col_f_nvzm1/2,0:col_f_nthm1,  &
                               0:col_f_nvrm1,col_f_sp_s:col_f_sp_e) :: f
      real(kind=8), intent(in), dimension(col_f_sp_s:col_f_sp_e) :: mass_in, charge_in
      real(kind=8), intent(in), dimension(1:col_f_npsi,col_f_sp_s:col_f_sp_e) :: den_in, t_ev_in
      real(kind=8), intent(out),dimension(0:col_f_nthm1,1:col_f_npsi,col_f_sp_s:col_f_sp_e) :: int_n, int_T,int_p
      real(kind=8), intent(out),dimension(1:col_f_npsi,col_f_sp_s:col_f_sp_e) :: FSAn, FSAT,FSAp
      ...
      !$omp shared(isize,  col_f_npsi,sml_istep,FSAn,FSAT,FSAp,J,int_J, col_f_dth,&
      !$omp        col_sp_cell_all, col_f_mat_list, col_f_ksp_list, col_f_vecb_list,dphidth,&
      !$omp        col_f_vecx_list, col_f_sp_num, gammac_spall, M_s, M_ab,invJ,B,dBdth, &
      !$omp        col_f_sp_s, col_f_sp_e, col_f_nvrm1, sml_mype, f_in,int_n,int_T,int_p,     &
      !$omp        mass_in, den_in, charge_in, t_ev_in,df_out, col_f_nthm1,f,opt_Col)             &
      ...
      do ipsi=1,isize ! isize = outer thread num?????
        ...
        do ith=0,col_f_nthm1
          call setup_col_sp( f, ith, mass_in, t_ev_in(ipsi,:), den_in(ipsi,:),  &
                           charge_in, col_sp_cell_all(:,ipsi), gammac_spall(:,:,ipsi))
        !!##### TIME ADVANCE
          select case (opt_col)
            case(0,1)
              if (col_f_sp_num.eq.1) then !!## Single Species Collisions
                call col_fm_core(col_sp_cell_all(:,ipsi),gammac_spall(:,:,ipsi),update_ok,sml_istep,invJ(ith,ipsi),B(ith,ipsi),dBdth(ith,ipsi),&
                  dphidth(ith,ipsi),col_f_mat_list(ipsi),col_f_ksp_list(ipsi),col_f_vecb_list(ipsi),col_f_vecx_list(ipsi),M_s(:,:,:,:,ipsi))
              else !!## Multi-Species Collisions
                call col_fm_core(col_sp_cell_all(:,ipsi),gammac_spall(:,:,ipsi),update_ok,sml_istep,invJ(ith,ipsi),B(ith,ipsi),dBdth(ith,ipsi),&
                  dphidth(ith,ipsi),col_f_mat_list(ipsi),col_f_ksp_list(ipsi),col_f_vecb_list(ipsi),col_f_vecx_list(ipsi),M_s(:,:,:,:,ipsi),M_ab(:,:,:,:,:,ipsi))
              end if
          ...
          if (update_ok) then
            do isp = col_f_sp_s, col_f_sp_e
              ...
              FSAn(ipsi,isp)=FSAn(ipsi,isp)+int_n(ith,ipsi,isp)*J(ith,ipsi)*col_f_dth
              FSAp(ipsi,isp)=FSAp(ipsi,isp)+int_p(ith,ipsi,isp)*J(ith,ipsi)*col_f_dth
              FSAT(ipsi,isp)=FSAT(ipsi,isp)+int_T(ith,ipsi,isp)*J(ith,ipsi)*col_f_dth
              do imu=0, col_f_nvrm1
                df_out(:,ith,imu,ipsi,isp) = col_sp_cell_all(isp,ipsi)%pdf_np2(imu+1,:)  !! at the end of col_fm_core, this is "df" we need
              enddo
            enddo
          endif
        enddo
        do isp = col_f_sp_s,col_f_sp_e
          FSAn(ipsi,isp)=FSAn(ipsi,isp)/int_J(ipsi)
          FSAT(ipsi,isp)=FSAT(ipsi,isp)/int_J(ipsi)
          FSAp(ipsi,isp)=FSAp(ipsi,isp)/int_J(ipsi)
        enddo
      enddo
!$omp end do
!$acc wait
...
!#endif
    ...
    subroutine col_fm_core(col_spall,gammac,update_ok,sml_istep,invJ,B,dBdth,dphidth,&
      ...
      use FP4D_globals, only: sml_mype, neg_frac
      implicit none
      integer :: col_f_mat
      integer :: col_f_ksp
      integer :: col_f_vecb
      ...
      real (kind=8), dimension(col_f_nvr, col_f_nvz) :: df,df2   ! local
      real (kind=8), dimension(col_f_nvr, col_f_nvz) :: df_corr,df_corr2   ! local
      real (kind=8), dimension(col_f_ntotal_v, col_f_sp_s:col_f_sp_e) :: dist_col, dist_col2
      ...
      real (kind=8), dimension(5,col_f_nvrm1,col_f_nvzm1) :: ED2
      ...
      real (kind=8), dimension(LU_nnz, col_f_sp_s:col_f_sp_e) :: LU_values,LU_values2 ! Super LU Variables
      ...
      do iter_inter=1, vpic_inner_iter_max
        ...
        !$omp& shared(dist_col,dist_col2,col_spall)                           &
        ...
        do spi=col_f_sp_s, col_f_sp_e
           do index_2=1,col_f_nvz
             do index_1=1,col_f_nvr
               ...
               dist_col2(index_0,spi) = col_spall(spi)%pdf_n(index_1,index_2)
             enddo
           enddo
        enddo
        ...
        do spi=col_f_sp_s, col_f_sp_e
           do spj=col_f_sp_s, col_f_sp_e
              ...
              select case (opt_col)
                case (0,1)
                  if(spi .eq. spj) then
                    call col_fm_E_and_D_s(col_spall(spi),fhalf,dfdr,dfdz,M_s(:,:,:,spi),ED,invJ,B,dBdth,dphidth,gammac) ! i-i
                    call col_fm_E_and_D_s2(col_spall(spi),fhalf,dfdr,dfdz,M_s(:,:,:,spi),ED2,invJ,B,dBdth,dphidth,gammac)
                  else
                    if(spj .gt. spi) then
                      spj_mod = spj-1
                    else
		    ...
		    spj_mod= spj
              ...
              call col_f_LU_matrix(spj, col_spall(spi), ED2, LU_values_tmp)
              LU_values2(:,spi) = LU_values2(:,spi) + LU_values_tmp*gammac(spj,spi)
           enddo !spj
        enddo !spi
        ...
        do spi=col_f_sp_s, col_f_sp_e
           ...
           call col_fm_picard_step(iter_inter, LU_values2(:,spi), dist_col2(:,spi), col_spall(spi)%pdf_np2)
...
!        if(iter_inter .eq. 1 ) then    !! Why did we set a condition for iter_inter?
...
!#if !defined(_OPENMP) && !defined(_OPENACC)
              ...
              ! possibly OpenACC
              write(*,'(a)') 'Not converged by collisions because:'
...
!#if !defined(_OPENMP) && !defined(_OPENACC)
              write(*,'(a)') 'Collision conservation mistmatch! i.e. numerical model is implemented in a wrong way.  &
          ...
          do spi=col_f_sp_s, col_f_sp_e
            ...
            do index_2=1,size(df_corr,2)
              do index_1=1,size(df_corr,1)
                ...
                df_corr2(index_1,index_2)=0.5D0*(sign(1D0,col_spall(spi)%pdf_np2(index_1,index_2))+1D0)
              enddo
            enddo
...
!#endif
              ...
              do index_2=1,size(df,2)
                do index_1=1,size(df,1)
                  ...
                  df2(index_1,index_2) = df_corr2(index_1,index_2) * &
                                       (col_spall(spi)%pdf_np2(index_1,index_2) - col_spall(spi)%pdf_n(index_1,index_2))
                enddo
              enddo
            endif
            ...
            do index_2=1,size(df,2)
              do index_1=1,size(df,1)
                ...
                col_spall(spi)%pdf_np2(index_1,index_2) = df2(index_1,index_2)
              enddo
            enddo
        ...
        if (update_ok) then
          do spi=col_f_sp_s, col_f_sp_e
            do ir=1,col_f_nvr
              ...
              col_spall(spi)%pdf_np2(ir,:) = col_spall(spi)%pdf_np2(ir,:)/col_spall(spi)%conv_factor
            enddo
          enddo
        endif
      endif 
...
!$acc wait(istream)
    ...
    subroutine col_fm_core_init(spa)
      use FP4D_globals, only : sml_istep, sml_e_charge, sml_mype
#ifdef _OPENMP
...
!$acc wait(istream)
    ...
    !!
    subroutine col_fm_E_and_D_s(cs,f_half,dfdr,dfdz,Ms,EDs,invJ,B,dBdth,dphidth,gammac)
      !cs1 : colliding, cs2 : target
      !f_half, dfdr, dfdz => cs2
      implicit none
...
!$omp parallel do default(none)                                      &
!$omp& collapse(2)                                                   &
!$omp& shared(mesh_Nrm1,mesh_Nzm1,f_half,dfdr,dfdz,Ze,dphidth,gammac,spid)       &
!$omp& shared(Ms,EDs,invJ,B,dBdth,mass,cs_mesh_r_half,cs_mesh_z_half)&
...
!$acc  wait(istream)
   ...
   subroutine col_fm_E_and_D_s2(cs,f_half,dfdr,dfdz,Ms,EDs,invJ,B,dBdth,dphidth,gammac)
      !cs1 : colliding, cs2 : target
      !f_half, dfdr, dfdz => cs2
      implicit none
      type(col_f_sp_type) :: cs
...
!          EDs(3,index_J,index_I) =  0.0
          EDs(4,index_J,index_I) =  sum4 &
            + 0.5*cs_mesh_r_half(index_J)*cs_mesh_z_half(index_I)*dBdth/B/B*invJ
          EDs(5,index_J,index_I) =  sum5 &
            - 0.5*cs_mesh_r_half(index_J)*cs_mesh_r_half(index_J)*dBdth/B/B*invJ &
